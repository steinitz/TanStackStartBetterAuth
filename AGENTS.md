...
- **CHECKS**: Verification gate = `pnpm test:run` + `tsc --noEmit`. Use those exact scripts: bare `pnpm exec tsc` silently no-ops, and `pnpm dlx tsc` false-fails on the deprecated `baseUrl` in tsconfig. Noninteractive/agent runs: prefix `CI=true pnpm test:run` (pnpm's run-wrapper can otherwise stop on a modules-purge prompt before vitest starts), or call `./node_modules/.bin/vitest run [file]` directly to skip the wrapper. Run one file with a plain arg (`pnpm test:run <file>`), not `-- <file>` — vitest won't treat that as a filter.

 ...
 * User is created with credentials but NOT authenticated — the test needs
 * to navigate the sign-in page to reach the "Reset Password" link.
 */
test.describe('Password Reset Flow', () => {
  ...
  test('should complete password reset flow from signin page', async ({ page }) => {
    // Setup: Create user with credentials (NOT authenticated — we need the sign-in page)
    const { email: testEmailAddress } = await createUserWithCredentials({
      password: testConstants.defaultPassword
    });
    // Clear emails after user creation (createUserWithCredentials sends none,
